...
import lk.ijse.elitedrivingschoolsystemormcoursework.util.PasswordEncryption;
...
public class LoginPageController implements Initializable {
    ...
    public void btnLoginOnAction(ActionEvent actionEvent) {
        ...
        String inputPassword = txtPassword.isVisible() ? txtPassword.getText() : txtVisiblePassword.getText();
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        txtVisiblePassword.textProperty().addListener((observable, oldValue, newValue) -> validFields());
        btnLogIn.setDisable(true);
    }

    private void validFields() {
        boolean isValidUsername = txtUsername.getText().matches(usernameRegex);
        String pwd = txtPassword.isVisible() ? txtPassword.getText() : txtVisiblePassword.getText();
        boolean isValidPassword = pwd.matches(passwordRegex);
        ...
        txtVisiblePassword.setStyle("-fx-border-color: #7367F0; -fx-border-radius: 12px; -fx-background-radius: 12px;");
        ...
        if (!isValidPassword) {
            if (txtPassword.isVisible())
                ...
                txtVisiblePassword.setStyle("-fx-border-color: red; -fx-border-radius: 12px; -fx-background-radius: 12px;");
        }
